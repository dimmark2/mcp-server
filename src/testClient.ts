...
function buildEnv(): Record<string, string> {
  ...
  if (!env.PGHOST) env.PGHOST = "centerbeam.proxy.rlwy.net";
  if (!env.PGPORT) env.PGPORT = "13403";
  if (!env.PGUSER) env.PGUSER = "postgres";
  if (!env.PGDATABASE) env.PGDATABASE = "postgres";
  if (!env.PGPASSWORD) env.PGPASSWORD = "qDJqEEbhMrQThzXAKRgtIFzFVKsHSaio";
